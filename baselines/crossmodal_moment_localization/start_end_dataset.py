...
from utils.basic_utils import load_jsonl, load_json, l2_normalize_np_array, flat_list_of_lists, merge_dicts, uniform_feature_sampling
...
class Pseudo_query_StartEndDataset(Dataset):
    """
    Args:
        dset_name, str, ["tvr"]
        ctx_mode: str,
    ...
    """
    def __init__(self, dset_name, data_path, visual_pseudo_query_bert_or_handler, textual_pseudo_query_bert_path_or_handler,
                 sub_bert_path_or_handler, max_desc_len, max_ctx_len, vid_feat_path_or_handler, clip_length,
                 training_strategy='aug', ctx_mode="video", normalize_vfeat=True, normalize_tfeat=True, h5driver=None,
                 data_ratio=1):
        self.dset_name = dset_name
        self.data_path = data_path
        self.data_ratio = data_ratio
        self.visual_pseudo_query_bert_or_handler = visual_pseudo_query_bert_or_handler
        self.textual_pseudo_query_bert_path_or_handler = textual_pseudo_query_bert_path_or_handler
        ...
        if isinstance(visual_pseudo_query_bert_or_handler, h5py.File):
            self.visual_query_bert_h5 = visual_pseudo_query_bert_or_handler
        else:
            self.visual_query_bert_h5 = h5py.File(visual_pseudo_query_bert_or_handler, "r", driver=h5driver)

        if isinstance(textual_pseudo_query_bert_path_or_handler, h5py.File):
            self.textual_query_bert_h5 = textual_pseudo_query_bert_path_or_handler
        else:
            self.textual_query_bert_h5 = h5py.File(textual_pseudo_query_bert_path_or_handler, "r", driver=h5driver)
        ...
        assert len(self.data) == len(self.visual_query_bert_h5) and len(self.data) == len(self.textual_query_bert_h5), \
            "Check the target dataset files, [length info] data : {}, vpq : {}, tpq:{}".format(len(self.data),
                                                                                               len(self.visual_query_bert_h5),
                                                                                               len(self.textual_query_bert_h5))
        if self.data_ratio == -1:
            self.data = load_jsonl(data_path)[:87175]
        logger.info("Single stream, Pseudo queries, : {}, type of query : {}".format(len(self.data), self.training_strategy))

        if self.data_ratio != 1 and self.data_ratio != -1:
            n_examples = int(len(self.data) * data_ratio)
            # self.data = random.sample(self.data, n_examples)
            self.data = self.data[:n_examples]
            logger.info("Using {}% of the summary data: {} examples".format(data_ratio * 100, n_examples))
    ...
    def __getitem__(self, index):
        ...
        meta = dict(desc_id=raw_data["desc_id"], pseudo_vid_query=raw_data["pseudo_vid_query"],
                    pseudo_sub_query=raw_data["pseudo_sub_query"], vid_name=raw_data["vid_name"], ts=raw_data["ts"])
        model_inputs = dict()

        if self.training_strategy == 'visual':
            model_inputs['query_feat'] = self.get_visual_query_feat_by_desc_id(meta["desc_id"])

        elif self.training_strategy == 'textual':
            model_inputs['query_feat'] = self.get_summary_feat_by_desc_id(meta["desc_id"])

        elif self.training_strategy == 'aug':
    ...
    def get_summary_feat_by_desc_id(self, desc_id):
        query_feat = self.textual_query_bert_h5[str(desc_id)][:self.max_desc_len]
        if self.normalize_tfeat:
            query_feat = l2_normalize_np_array(query_feat)
        return torch.from_numpy(query_feat)

    def get_visual_query_feat_by_desc_id(self, desc_id):
        query_feat = self.visual_query_bert_h5[str(desc_id)][:self.max_desc_len]
        if self.normalize_tfeat:
            query_feat = l2_normalize_np_array(query_feat)
        return torch.from_numpy(query_feat)
