...
def load_preprocess_tvr_verb_bert_query(tvr_file_path):
    return [dict(id=e["desc_id"], text=e["simplified_sen"]) for e in load_jsonl(tvr_file_path)]

def load_preprocess_tvr_textual_pseudo_query(tvr_file_path):
    return [dict(id=e["desc_id"], text=e["pseudo_sub_query"]) for e in load_jsonl(tvr_file_path)]
    # return [dict(id=e["desc_id"], text=e["dialogue"]) for e in load_jsonl(tvr_file_path)]

def load_preprocess_tvr_visual_pseudo_query(tvr_file_path):
    return [dict(id=e["desc_id"], text=e["pseudo_vid_query"]) for e in load_jsonl(tvr_file_path)]
...
def chunk_single_sub(tokenizer, single_sub_data, max_length=256, drop_edge=True):
    ...
    num_chunks = np.ceil(tokenized_lengths_cumsum[-1] / max_length).astype(np.int16)
...
def load_and_cache_examples(tokenizer, data_path, max_length, pseudo_query_data_path=None, verb_bert_data_path=None, sub_data_file=None, filter_file_path=None,
                            load_query=True, load_sub=False, load_visual_pseudo_query=False, load_textual_pseudo_query=False, load_verb_bert=False, extract_sub=False, add_extra_keys=False, debug=False):

    assert load_query or load_sub or load_visual_pseudo_query or load_verb_bert or load_textual_pseudo_query, "Must specify one of them"
    if extract_sub:
        assert not load_query, "Do not load query when extracting for subtitles"

    logger.info("Now Loading query : {}, subtitles : {}, textual_pseudo_query : {}, visual_pseudo_query : {}, verb bert : {}".format(
        load_query, load_sub, load_textual_pseudo_query, load_visual_pseudo_query, load_verb_bert))
    ...
    if load_visual_pseudo_query:    # add summary data for training as well
        logger.info("Load visual pseudo query")
        input_datalist += flat_list_of_lists([load_preprocess_tvr_visual_pseudo_query(pseudo_query_data_path)])

    if load_textual_pseudo_query:
        logger.info("Load textual pseudo query")
        input_datalist += flat_list_of_lists([load_preprocess_tvr_textual_pseudo_query(pseudo_query_data_path)])

    if load_verb_bert:
        input_datalist += flat_list_of_lists([load_preprocess_tvr_verb_bert_query(verb_bert_data_path)])
    ...
    if debug:
        logger.info("  **** Input Examples ****")
        logger.info("1. {}".format(input_datalist[0]))
...
def extract(args, model, tokenizer, prefix=""):
    ...
    if args.use_textual_pseudo_query:
        extract_dataset = load_and_cache_examples(tokenizer, args.train_data_file, args.block_size,
                                                  pseudo_query_data_path=args.pseudo_query_data_path,
                                                  sub_data_file=args.sub_data_file,
                                                  filter_file_path=None,
                                                  load_query=not args.use_textual_pseudo_query,
                                                  load_sub=args.use_sub,
                                                  load_textual_pseudo_query=args.use_textual_pseudo_query,
                                                  extract_sub=args.use_sub,
                                                  add_extra_keys=args.use_sub,
                                                  debug=args.debug)

    elif args.use_visual_pseudo_query:
        extract_dataset = load_and_cache_examples(tokenizer, args.train_data_file, args.block_size,
                                                  pseudo_query_data_path=args.pseudo_query_data_path,
                                                  sub_data_file=args.sub_data_file,
                                                  filter_file_path=None,
                                                  load_query=not args.use_visual_pseudo_query,
                                                  load_sub=args.use_sub,
                                                  load_visual_pseudo_query=args.use_visual_pseudo_query,
                                                  extract_sub=args.use_sub,
                                                  add_extra_keys=args.use_sub,
                                                  debug=args.debug)

    elif args.use_verb_bert:
        extract_dataset = load_and_cache_examples(tokenizer, args.train_data_file, args.block_size,
                                                  pseudo_query_data_path=args.verb_bert_data_file,
                                                  sub_data_file=args.sub_data_file,
                                                  filter_file_path=None,
                                                  load_query=not args.use_verb_bert,
                                                  load_sub=args.use_sub,
                                                  load_verb_bert=args.use_verb_bert,
                                                  extract_sub=args.use_sub,
                                                  add_extra_keys=args.use_sub,
                                                  debug=args.debug)
...
def main():
    ...
    parser.add_argument("--use_textual_pseudo_query", action="store_true",
                        help="At finetuning: additionally add subtitles associated with the given train_data_file. "
                             "At extraction: extract embeddings for summary")
    parser.add_argument("--use_verb_bert", action="store_true",
                        help="At extraction: extract embeddings for verb bert query")
    parser.add_argument("--use_visual_pseudo_query", action="store_true",
                        help="At extraction: extract embeddings for visual pseudo query")
    parser.add_argument("--sub_data_file", type=str,
                        help="subtitle file, when --do_extract and --use_sub is set, extract from this file")
    parser.add_argument("--pseudo_query_data_path", type=str,
                            help="path to the pseudo-supervision path")
    parser.add_argument("--verb_bert_data_file", type=str,
                            help="path to the queries generated by Verb BERT.")
    ...
    if args.do_train:
        ...
        if args.pseudo_query_data_path is not None:
            args.use_visual_pseudo_query = True
            args.use_textual_pseudo_query = True
